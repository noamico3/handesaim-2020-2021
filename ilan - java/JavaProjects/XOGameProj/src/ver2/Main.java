...
public class Main
{
    ...
    private static void createGUI()
    {
        ...
        //----------------------------הוספת כפתור למעלה ושינוי צבע--------------------------------
        JButton btn_click = new JButton("click me");
        btn_click.setFocusable(false);
        btn_click.setForeground(Color.red);
        btn_click.setFont(FONT2);
        ...
        btnNewGame.addActionListener(new ActionListener()
        {
            ...
            {
                newGame();
                //lblTurn.setBackground(Color.MAGENTA);
            }
        });
    ...
    private static void cleanBoard()
    {
        for(int row=0; row < ROWS; row++)        
        {
            for(int col=0; col < COLS; col++)            
            {
                ...
                btnMat[row][col].setBackground(null);//חייב כי הצביעה של הניצחון הקודם תשאר אם לא
            }
        }
    }
